...
# MAGIC
# MAGIC This is a basic SQL query like many others we have written so far.

# COMMAND ----------

# MAGIC %sql
# MAGIC SELECT p_name, p_retailprice
# MAGIC FROM samples.tpch.part
# MAGIC WHERE p_retailprice < 1000 
# MAGIC ORDER BY p_retailprice DESC
...
# MAGIC %md
# MAGIC
# MAGIC The example from the beginning of this lesson used the SparkSession method **`table`** to create a DataFrame from the **`part`** table available in the **`sample `** catalog. Let's save this in the variable **`parts_df`**.

# COMMAND ----------

parts_df = spark.table("samples.tpch.part")
